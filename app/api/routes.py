...
def worker_heartbeat(worker_id: str = "local", version: str = ""):
    db.set_setting("worker_last_seen", {"worker_id": worker_id, "ts": time.time(), "version": version})
    return {"ok": True, "server_time": time.time()}


@ui.get("/worker/status")
def worker_status():
    hb = db.get_setting("worker_last_seen", None)
    queued = db.q("SELECT kind, COUNT(*) n, state FROM job_queue GROUP BY kind, state")
    return {"heartbeat": hb, "online": bool(hb and time.time() - hb.get("ts", 0) < 120),
            "queue": queued}
